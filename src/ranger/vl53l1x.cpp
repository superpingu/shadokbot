...
{
  Wire1.beginTransmission(address);
  Wire1.write((RESULT__RANGE_STATUS >> 8) & 0xFF); // reg high byte
  Wire1.write( RESULT__RANGE_STATUS       & 0xFF); // reg low byte
  last_status = Wire1.endTransmission();

  Wire1.requestFrom(address, (uint8_t)17);

  results.range_status = Wire1.read();

  Wire1.read(); // report_status: not used

  results.stream_count = Wire1.read();

  results.dss_actual_effective_spads_sd0  = (uint16_t)Wire1.read() << 8; // high byte
  results.dss_actual_effective_spads_sd0 |=           Wire1.read();      // low byte

  Wire1.read(); // peak_signal_count_rate_mcps_sd0: not used
  Wire1.read();

  results.ambient_count_rate_mcps_sd0  = (uint16_t)Wire1.read() << 8; // high byte
  results.ambient_count_rate_mcps_sd0 |=           Wire1.read();      // low byte

  Wire1.read(); // sigma_sd0: not used
  Wire1.read();

  Wire1.read(); // phase_sd0: not used
  Wire1.read();

  results.final_crosstalk_corrected_range_mm_sd0  = (uint16_t)Wire1.read() << 8; // high byte
  results.final_crosstalk_corrected_range_mm_sd0 |=           Wire1.read();      // low byte

  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0  = (uint16_t)Wire1.read() << 8; // high byte
  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 |=           Wire1.read();      // low byte
}
